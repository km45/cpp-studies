...
// Utility
//-----------------------------------------------------------------------------
#define EXPECT_POD(TYPE) \
  static_assert(std::is_pod<TYPE>::value, #TYPE " should be POD.")

#define EXPECT_NON_POD(TYPE) \
  static_assert(!std::is_pod<TYPE>::value, #TYPE " should be non-POD.")
